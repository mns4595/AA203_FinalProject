...
class RRT(object):
    ...
    def __init__(self, statespace_lo, statespace_hi, x_init, x_goal, obstacles, testing):
    ...
    def solve(self, eps, max_iters=1000, goal_bias=0.05, shortcut=False):
        ...
        for k in range(max_iters):
            ...
            if z < goal_bias:
                ...
                xr = np.random.rand(state_dim)*(self.statespace_hi - self.statespace_lo) + self.statespace_lo

            xNear_index = self.find_nearest(V[0:n,:], xr)
            xNear = V[xNear_index, :]
            xn = self.steer_towards(xNear, xr, eps)

            if self.is_free_motion(self.obstacles, xNear, xn):
                V[n,:] = xn
                P[n] = xNear_index
                n = n+1

                if np.array_equal(xn, self.x_goal):
                    m = n-1
                    self.path.append(self.x_goal)
                    while not np.array_equal(V[m,:], self.x_init):
                        self.path.append(V[P[m], :])
                        m = P[m]
    ...
    def shortcut_path(self):
        ...
        while not success:
            ...
            for i in range(len(self.path)) :
                x = self.path[i]
                if not np.array_equal(x, self.x_init) and not np.array_equal(x, self.x_goal):
                    parent_index = i - 1
                    child_index = i + 1
                    if self.is_free_motion(self.obstacles, self.path[parent_index], self.path[child_index]):
                        del self.path[i]
                        success = False
                        break
...
class GeometricRRT(RRT):
    ...
    def find_nearest(self, V, x):
        ...
        for i in range(len(V[:,0])):
            dist = np.sqrt(np.power(V[i,0] - x[0], 2) + np.power(V[i,1] - x[1], 2))
...
class DubinsRRT(RRT):
    ...
    def steer_towards(self, x1, x2, eps):
        ...
        temp_length = path_length(x1, x2, self.turning_radius)

        if temp_length < eps:
            return x2

        dublins_path = path_sample(x1, x2, 1.001*self.turning_radius, eps)

        path_segments = dublins_path[0]
